...
class Request extends Service
{
    ...
    {
        parent::__construct();
        $this->httpRequest->authenticate(
            (string)$config->getSetting('github.user'),
            (string)$config->getSetting('github.token')
        );
    }
    ...
    public function getReleases(): Releases
    {
        ...
        do {
            $this->httpRequest->get(
                'https://api.github.com/repos/feastframework/framework/releases?page=' . (string)$page
            );
            $this->httpRequest->makeRequest();
//            var_dump($this->httpRequest->getResponseAsString());

            /** @var array<array-key, \stdClass>|\stdClass $response */
            $response = $this->httpRequest->getResponseAsJson();
            if (empty($response)) {
                $empty = true;
            }
            if (!is_array($response)) {
                throw new GithubException('Error fetching documentation from Github. See Readme for more details', $response);
            }
